...
class Keyboard:
    def __init__(self, io_controller):
        ...
        # Расширяем матрицу клавиатуры до 8x8 для Spectrum 128
        self.keyboard_matrix = np.zeros((8, 8), dtype=bool)
        self.keymap = {
            ...
            # Добавляем дополнительные клавиши для Spectrum 128
            pygame.K_BACKSPACE: (0, 5), pygame.K_LEFT: (0, 6), pygame.K_DOWN: (0, 7),
            pygame.K_UP: (1, 5), pygame.K_RIGHT: (1, 6), pygame.K_F3: (1, 7),
            pygame.K_F4: (2, 5), pygame.K_F1: (2, 6), pygame.K_F2: (2, 7),
            pygame.K_F5: (3, 5), pygame.K_ESCAPE: (3, 6), pygame.K_TAB: (3, 7),
    ...
    def read_port_fe(self, port):
        ...
        for row in range(8):
            ...
            if keyboard_line & cur_line != 0:
                row_value = 0
                for col in range(8):  # Изменено с 5 на 8 для поддержки 8 столбцов
                    if self.keyboard_matrix[row, col]:
                        row_value |= (1 << col)
                result &= ~row_value & 0xFF  # Изменено с 0x1F на 0xFF для поддержки 8 бит
        return result
    ...
    def display_keyboard(self, screen, font, offset):
        x, y = 10, 10 + offset
        row_text = "KEYBOARD (ZX Spectrum 128)"
