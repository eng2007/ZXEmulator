...
if __name__ == '__main__':
    ...
    for t, results in zip(tests_in.split("\n\n"), tests_expected.split("\n\n")):
        ...
        try:    
            ...
            regsr = ['ix', 'iy', 'af_prime', 'de_prime', 'bc_prime', 'hl_prime']
            print ("Registers:")
            for rl, rr in zip(regs, regsr):
                #print (rl, ": {0:X}".format( mach.registers[rl]), "\t\t", rr, ": {0:X}".format( mach.registers[rr]))
                print  (rl, ": {0:4X}".format( mach.get_register_value(rl)), "\t\t", rr, ": {0:4X}".format(mach.get_register_value(rr) ))                
            raise
        print ("PASSED")
        passes += 1
