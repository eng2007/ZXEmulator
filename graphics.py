...
class ZX_Spectrum_Graphics:
    def __init__(self, memory, pixel_size):
        ...
        self.scr_addr = np.zeros((self.screen_width, self.screen_height, 3), dtype=np.uint16)
    ...
    def fill_scr_addr(self):
        for y in range(self.screen_height):
            for x in range(self.screen_width):
                byte_index = x // 8   
                address = self.scr_base_address + ((y & 0b11000000) << 5  ) | ((y & 0b00111000) << 2 )  | ((y & 0b00000111 ) << 8)  + byte_index   
                attribute_address = self.scr_base_address + 6144 + ((y // 8) * 32) + (byte_index)
    ...
    def set_pixel(self, x, y, color_index):
        ...
        line_offset = ((y & 0b11000000)  ) | ((y & 0b00111000) >> 3 )  | ((y & 0b00000111 ) << 3)
    ...
    def set_attribute(self, x, y, ink, paper, bright=False):
        ...
        attribute_address = self.scr_base_address + 6144 + ((y // 8) * 32) + (x // 8)
    ...
    def render_screen_fast2(self):
        ...
        for y in range(self.screen_height):
            address_y = self.scr_base_address + ((y & 0b11000000) << 5  ) | ((y & 0b00111000) << 2 )  | ((y & 0b00000111 ) << 8)
        ...
        for y in range(self.screen_height):
            for x in range(0, self.screen_width, 8):
                ...
                for bit in range(8):
                    ...
                    pygame.draw.rect(self.screen, color, (xs * self.pixel_size, y * self.pixel_size, self.pixel_size, self.pixel_size))
                    #buffer[xs, y] = color
    ...
    def load_screen(self, pixel_data, attribute_data):
        ...
        self.memory[self.scr_base_address:self.scr_base_address+6144] = pixel_data
