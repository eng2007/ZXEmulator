...
class baseCPUClass:
	...
	def im(self,mode):
		...
		self.interrupt_mode=mode;self.registers['IM']=mode
	...
	def handle_interrupt(self):
		...
		elif self.interrupt_mode==2:vector=0;address=self.registers['I']<<8|vector;self.registers['PC']=self.memory[address+1]<<8|self.memory[address]
	...
	def fetch(self):self.registers['R']=self.registers['R']+1&127|self.registers['R']&128;value=self.memory[self.registers['PC']];self.registers['PC']=self.registers['PC']+1&65535;return value
	...
	def update_flags(self,result,zero=False,sign=False,parity=False,halfcarry=True,carry=False):
		...
		if parity:self.set_flag('P/V',self.parity(result))
		if halfcarry:self.set_flag('H',self.registers['F']&~16|(16 if result&15<self.registers['A']&15 else 0))
		if carry:self.set_flag('C',result>255)
		self.set_flag('N',0)
	def io_read(self,port):return self.io_controller.read_port(port)
	def io_write(self,port,value):self.io_controller.write_port(port,value)
	...
	def get_register_pair(self,pair):
		...
		elif pair=='IX':return self.registers['IX']
		elif pair=='IY':return self.registers['IY']
		else:high,low=pair;return self.registers[high]<<8|self.registers[low]
	def set_register_pair(self,pair,value):
		...
		if pair=='SP':self.registers['SP']=value
			...
			if len(pair)==2:high,low=pair
			else:high=pair[0]+'_';low=pair[1]+'_'
			self.registers[high]=value>>8&255;self.registers[low]=value&255
	...
	def add(self,operand):value=self.registers[operand]if isinstance(operand,str)else operand;result=self.registers['A']+value;h_flag=(self.registers['A']&15)+(value&15)&16==16;overflow=(self.registers['A']^result)&(value^result)&128!=0;self.registers['A']=result&255;self.update_flags(result,zero=True,sign=True,carry=True,halfcarry=True);self.set_flag('H',h_flag);self.set_flag('P/V',overflow);self.set_flag('3',self.registers['A']&8);self.set_flag('5',self.registers['A']&32)
	...
	def xor_a(self,operand):
		...
		result=self.registers['A']^value;self.registers['A']=result&255;self.set_flag('S',result&128);self.set_flag('Z',result==0);self.set_flag('H',0);self.set_flag('P/V',self.parity(result));self.set_flag('N',0);self.set_flag('C',0);self.set_flag('3',self.registers['A']&8);self.set_flag('5',self.registers['A']&32)
	...
	def cp(self,value):
		...
		result=self.registers['A']-operand&255;self.set_flag('S',result&128);self.set_flag('Z',result==0);self.set_flag('H',(self.registers['A']&15)-(operand&15)&16);self.set_flag('P/V',(self.registers['A']^operand)&(self.registers['A']^result)&128!=0);self.set_flag('N',1);self.set_flag('C',self.registers['A']<operand);self.set_flag('3',operand&8);self.set_flag('5',operand&32)
	def and_a(self,value):
		if isinstance(value,str):operand=self.registers[value]
		else:operand=value
		result=self.registers['A']&operand;self.registers['A']=result;self.set_flag('S',result&128);self.set_flag('Z',result==0);self.set_flag('H',1);self.set_flag('P/V',self.parity(result));self.set_flag('N',0);self.set_flag('C',0);self.set_flag('3',self.registers['A']&8);self.set_flag('5',self.registers['A']&32)
	...
	def sub(self,operand):
		...
		result=self.registers['A']-value;self.set_flag('H',(self.registers['A']&15)-(value&15)<0);self.set_flag('C',result<0);self.set_flag('P/V',(self.registers['A']^value)&(self.registers['A']^result&255)&128!=0);self.registers['A']=result&255;self.set_flag('S',self.registers['A']&128);self.set_flag('Z',self.registers['A']==0);self.set_flag('N',1);self.set_flag('3',self.registers['A']&8);self.set_flag('5',self.registers['A']&32)
