...
class extCPUClass(baseCPUClass):
    ...
    def dec_index_d(self,index_reg):offset=self.fetch_signed();address=self.registers[index_reg]+offset&65535;value=self.memory[address];result=value-1&255;self.memory[address]=result;self.set_flag('S',result&128);self.set_flag('Z',result==0);self.set_flag('H',value&15==0);self.set_flag('P/V',value==128);self.set_flag('N',1)
